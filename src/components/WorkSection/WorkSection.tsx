...
const WorksSection = () => {
  ...
  const visibleSlides = Math.min(3, total); // Don't show more slides than available items
  ...
  for (let i = 0; i < visibleSlides; i++) {
    if (total > 0) {
      visible.push(filteredImages[(current + i) % total]);
    }
  }
        ...
        {total > 0 ? (
          ...
          <div className="text-center py-12">
            <p className="text-slate-600 text-lg">
              Работы по категории "{activeFilter}" пока не добавлены
            </p>
          </div>
        )}
